...
const Home = () => {
  ...
  const { data: subDestinationData, refetch: subDestinationRefetch } = useQuery<
    ISubDestination[]
  >('specificsubDestination', () =>
    axios
      .get(`api/subDestinations/${searchDestination}`)
      .then((res) => res.data)
  );
  return (
    <div>
      <Container maxWidth="lg">
        <Banner setSearchDestination={setSearchDestination} />
        {searchDestination ? (
          <Card
            sx={{
              padding: '25px',
